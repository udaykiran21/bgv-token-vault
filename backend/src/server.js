...
try {
  ...
  let artifactPath = path.join(
    __dirname,
    "../blockchain/artifacts/contracts/EmploymentRegistry.sol/EmploymentRegistry.json"
  );
  ...
  console.warn(
    "Contract artifact not found. Please compile the hardhat project."
  );
}
...
app.post("/api/hr/verify", (req, res) => {
  ...
  if (VALID_GSTINS.has(gstin)) {
    ...
    res
      ...
      .json({
        success: false,
        message: "Invalid or Unregistered Company GSTIN",
      });
  }
});
...
app.post("/api/record/create", async (req, res) => {
  try {
    ...
    if (!VALID_GSTINS.has(gstin)) {
      return res
        .status(401)
        .json({ success: false, message: "Unauthorized HR" });
    }
...
app.post("/api/record/grantAccess", async (req, res) => {
  try {
    ...
    const candidateWallet = new ethers.NonceManager(
      new ethers.Wallet(candidatePrivateKey, provider)
    );
...
app.get("/api/record/verify/:recordId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Access denied or record not found." });
  }
});
...
app.get("/api/document/:recordId", async (req, res) => {
  try {
    ...
    if (record.companyId !== gstin) {
      const hasAccess = await contract.accessPermissions(
        record.candidate,
        gstin,
        recordId
      );
